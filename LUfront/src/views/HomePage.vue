<template>
  <div class="back">
      ...
      <v-flex xs12 sm6 md4 lg4 v-for="book in books" :key="book.id">
        <v-card hover elevation="2" class="text-center ma-6">
          <div class="cardBorderColor">
            ...
            <v-card-actions>
              <template>
                <v-btn color="primary" @click="addToChart(book)">
                  Buy
    ...
    <v-btn color="primary" class="mt-12" @click="subscribe()">
      Subscribe to audio books
...
export default {
  ...
  methods: {
    ...
    subscribe() {
      window.open("http://localhost:8083/subscription");
    },
  },
  mounted() {
    axios
